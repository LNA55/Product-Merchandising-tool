      ...
      <div className="soft-card mt-4 overflow-x-auto">
        <table className="w-full min-w-[980px] text-[13px]">
          ...
          <tbody>
            {filtered.map((r) => {
              ...
              return (
                <tr key={r.id} className={`border-b border-stone-100 align-top ${r.status !== 'draft' ? 'bg-cyan-50/40' : ''}`}>
                  ...
                  <td className="max-w-[200px] px-4 py-3 font-medium text-stone-800">{r.name}</td>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    {r.estimation
                      ? <span className="text-cyan-800">Completed <span className="tabular-nums text-stone-500">({r.estimation.engagement} eng · {r.estimation.atc} ATC)</span></span>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={8} className="px-4 py-8 text-center text-sm text-stone-400">No release matches the current filters.</td></tr>
            )}
          </tbody>
        </table>
      </div>
