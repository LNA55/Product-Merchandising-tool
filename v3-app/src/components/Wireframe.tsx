...
export function Wireframe({ kind, hl, fold }: { kind: string; hl: [number, number]; fold?: boolean }) {
  ...
  return (
    <svg viewBox="0 0 320 210" className="w-full rounded-md border border-neutral-200 bg-white" role="img" aria-label="Position of the merch block in the page">
      ...
      <rect x={10} y={20} width={46} height={6} rx={2} fill="#0e7490" />
      ...
      <rect x={hx} y={hl[0]} width={hw} height={hl[1]} rx={3} fill="rgba(14,116,144,.14)" stroke="#0e7490" strokeWidth={2} strokeDasharray="7 4" />
      <text x={hx + 6} y={hl[0] + hl[1] - 6} fontSize={9} fill="#0e7490" fontFamily="sans-serif" fontWeight="bold">MERCH BLOCK</text>
    </svg>
  )
}
