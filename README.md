...
| `version-3/` + `v3-app/` | Current prototype (built output + source). Per-block releases (`MERCH_BLOCK_ID_Vn.x`), 1–10 weights with computed % shares, parameterised API calls per data source, Swagger sandbox, impact reports, merchandising doors, preferences (final user groups, roles & permissions, integrations). |
